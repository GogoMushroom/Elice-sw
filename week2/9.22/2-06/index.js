...
function highlight() {
    const word = getElementsByTagName("strong");
    word.map((str) => (str.style.color = "blue"));

    for(let i = 0 ; i < word.length; i++){
        word[i].style.color = "blue";   
    }
...
function returnNormal() {
    const word = document.querySelectorAll("strong");
    word.map((str) => (str.style.color = "block"));
}
